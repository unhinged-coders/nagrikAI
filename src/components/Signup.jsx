...
export default function Signup({ onComplete }) {
  ...
  const handleMobileCheck = async () => {
    ...
    try {
      ...
      if (!snap.empty) {
        // ── Returning user — MUST include id field ──
    ...
    try {
      ...
      if (!snap.empty) {
        ...
        userData = {
          ...
          reportsCount: 0,
          points:       0,
          id:           docRef.id,   // ← critical
        }
      }
      localStorage.setItem('nagrik_user', JSON.stringify(userData))
      onComplete(userData)
    } catch (e) { console.error(e); setError('Error saving data. Try again!') }
    setLoading(false)
  }
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sg-lang { display: flex; justify-content: center; margin-bottom: 20px; }
      ...
      <div className="sg-wrap">
        <div className="sg-card">
          ...
          <div className="sg-lang"><LangToggle /></div>

          {step === 'mobile' && (
            <>
              <div className="sg-sub">{t('signupSub')}</div>
              ...
              <div className="sg-divider">{t('mobileOnlyHint')}</div>
            </>
          )}

          {step === 'register' && (
            <>
              <button className="sg-back" onClick={() => { setStep('mobile'); setError('') }}>{t('backBtn')}</button>
              <div className="sg-mobile-display">📱 +91 {mobile}</div>
              <div className="sg-sub" style={{ marginBottom: 16 }}>{t('newUserSub')}</div>
              <input className="sg-input" placeholder={t('firstNamePlaceholder')} value={form.firstName} onChange={e => setForm({ ...form, firstName: e.target.value })} />
              <input className="sg-input" placeholder={t('lastNamePlaceholder')} value={form.lastName} onChange={e => setForm({ ...form, lastName: e.target.value })} />
              <input className="sg-input" placeholder={t('emailPlaceholder')} type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} />
              {error && <div className="sg-error">{error}</div>}
              <button className="sg-btn" onClick={handleRegister} disabled={loading}>
                {loading ? t('checking') : t('registerBtn')}
              </button>
            </>
          )}

          <div className="sg-privacy">{t('privacy')}</div>
        </div>
      </div>
    </>
  )
